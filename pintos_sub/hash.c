...
{
  return hash_int(hash_entry(e, struct hash_item, elem)->value);
}

bool hash_cmp (const struct hash_elem *a, const struct hash_elem *b,
 void *aux)
{
  return hash_entry(a, struct hash_item, elem)->value < hash_entry(b, struct hash_item, elem)->value;
}

void hash_destructor(struct hash_elem *e, void *aux)
{
  // if(e->list_elem.prev != NULL) free(e->list_elem.prev);
  // if(e->list_elem.next != NULL) free(e->list_elem.next);
  free(e);
}

void hash_action_square(struct hash_elem *e, void *aux)
{
  int val = hash_entry(e, struct hash_item, elem)->value;
  hash_entry(e, struct hash_item, elem)->value = val * val;
}

void hash_action_triple(struct hash_elem *e, void *aux)
{
  int val = hash_entry(e, struct hash_item, elem)->value;
  hash_entry(e, struct hash_item, elem)->value = val * val * val;
}
